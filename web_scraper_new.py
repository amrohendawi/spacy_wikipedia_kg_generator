...
# nlp_coref.vocab.vectors = nlp.vocab.vectors
...
def get_entity_pairs(text, coref_available=True):
    ...
    text = nlp(text)
    # if coref_available:
    #     text = nlp(text)  # resolve coreference clusters
    ...
    def refine_ent(ent, sent):
        ...
        if ent_type == '':
            ent_type = 'NOUN_CHUNK'
            ent = ' '.join(
                str(t.text) for t in nlp(str(ent.doc)) if t.pos_ not in unwanted_tokens and t.is_stop == False)
